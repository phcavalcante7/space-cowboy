...
@export var bullet_scene : PackedScene = preload("res://Bruxa/Scenes/bullet-witch.tscn")
@export var snowman_scene : PackedScene = preload("res://Bruxa/Snowman/Scenes/snowman.tscn")
...
func _physics_process(delta):
	if morreu or summoning or not inScene:
		return
	death()
	if navigation_agent.distance_to_target() <= 500 and not wall_verifier.is_colliding() and not morreu:
		print("aqui")
		shoot = true
...
func evocando_snowman():
	...
	get_node("AnimatedSprite2D").play("summon")
	...
	# - snowman 2 -
	var snowman2 = snowman_scene.instantiate()
	snowman2.position = self.global_position + Vector2(20,20)
	get_parent().add_child(snowman2)
	# - snowman 3 -
	var snowman3 = snowman_scene.instantiate()
	snowman3.position = self.global_position - Vector2(20,20)
	get_parent().add_child(snowman3)
	summoning = false
...
func _on_shoot_timeout():
	if navigation_agent.distance_to_target() <= 500 and not wall_verifier.is_colliding() and not summoning:
		spawn_bullets()
